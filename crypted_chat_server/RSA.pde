...
public class RSA{
  ...
  void savePublicKey(){
    String[] tekst = {this.publicKey.toString(), this.n.toString()};
 ...
 public void loadServerKey(){
  ...
  public Boolean serverKeyExists(){
    return checkIfFileExist("PublicKey.txt");
  }


  void saveRSA(){
    String[] tekst = {p.toString(), q.toString(), n.toString(), phi.toString(), publicKey.toString(), privateKey.toString()};
    saveStrings("rsa.txt", tekst);
  }
  ...
  public Boolean rsaExists(){
    return checkIfFileExist("rsa.txt");
  }

  public Boolean checkIfFileExist(String fileName){
    return createInput(fileName) != null;
  }

  public void loadRSA(){
    String[] linjer = loadStrings("rsa.txt");
    this.p = new BigInteger(linjer[0]);
    this.q = new BigInteger(linjer[1]);
    this.n = new BigInteger(linjer[2]);
    this.phi = new BigInteger(linjer[3]);
    this.publicKey = new BigInteger(linjer[4]);
    this.privateKey = new BigInteger(linjer[5]);
  }
  ...
  // Bruges til at kryptere besked til severen
  public BigInteger[] serverKryptere(String besked){
    return kryptereMedEnhverKey(serverPublicKey, serverN, besked);
  }
  
  
  // Kryptere med eget key
  public BigInteger[] kryptere(String besked){
    return kryptereMedEnhverKey(publicKey, n, besked);
  }
  
  // Kryptere med vilkårlig nøgle
  public BigInteger[] kryptereMedEnhverKey(BigInteger publicKey, BigInteger n, String besked){
